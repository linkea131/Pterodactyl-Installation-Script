...
preflight(){
    output ""
    output "Installation / Upgrade Script"
    output ""
...
install_options(){
    ...
    output "[1] Install the panel to 1.0.3 - Working"
    output "[2] Install the daemon - May cause issues"
    output "[3] Install the panel and daemon - May cause issues"
    output "[4] Install the standalone SFTP server - May cause issues"
    output "[5] Upgrade the panel to 1.0.3 - Working"
    output "[6] Upgrade the daemon to 0.6.13 - May cause issues"
    output "[7] Upgrade the panel to 1.0.3 and daemon to 0.6.13 - Working"
...
dns_check(){
    output "Please enter your FQDN (panel.domain.com):"
    read FQDN
...
install_daemon() {
    ...
    if [[ ( "$lsb_dist" =  "centos" ||  "$lsb_dist" =  "rhel" ) && "$dist_version" = "8" ]]; then
    	curl -sSL https://raw.githubusercontent.com/tommytran732/CentOS-8-Docker-Script/master/get-docker.sh | sudo bash #Docker
    else
        output "Installing Docker"
        curl -sSL https://get.docker.com/ | CHANNEL=stable bash #Docker
    fi
